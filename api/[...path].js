...
function isAdmin(req) {
  const h = String(req.headers.authorization || '');
  const t = h.startsWith('Bearer ') ? h.slice(7) : '';
  return !!t && state.adminTokens.has(t);
}
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (m && req.method === 'PUT') { const b=await body(req), id=decodeURIComponent(m[1]), i=state.orders.findIndex(x=>x.id===id); if(i<0)return json(res,404,{error:'غير موجود'}); state.orders[i].status=String(b.status || state.orders[i].status); return json(res,200,state.orders[i]); }
