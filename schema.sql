...
-- Public reads
drop policy if exists "public banners" on public.hero_banners;
create policy "public banners" on public.hero_banners for select using (active=true);
...
-- Existing installations: add the product gallery column if it is missing.
alter table public.products add column if not exists images jsonb not null default '[]'::jsonb;


-- V32 additions: product account requirements, banner overlays, support text, admin notifications.
...
-- Server-side service_role handles notification writes/reads.
